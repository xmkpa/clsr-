    ...
    def run_eval(self, filename, num_ngs):
        ...
        for batch_data_input in self.iterator.load_data_from_file(
            ...
            if batch_data_input:
                ...
                #把他改掉group_preds.extend(np.reshape(step_pred, (-1, group)))
                n = (len(step_pred) // group) * group
                if n > 0:
                    group_preds.extend(np.reshape(step_pred[:n], (-1, group)))
                    group_labels.extend(np.reshape(step_labels[:n], (-1, group)))

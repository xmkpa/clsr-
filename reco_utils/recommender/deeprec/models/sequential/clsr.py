...
class CLSRModel(SequentialBaseModel):
    ...
    def _compute_contrastive_loss(self):
        ...
        def reduce_intent_scores(scores):
            reduce_method = getattr(self.hparams, "contrastive_intent_reduce", "max")
            if reduce_method == "min":
                return tf.reduce_min(scores, axis=1)
            if reduce_method == "softmin":
                return -tf.reduce_logsumexp(-scores, axis=1)
            if reduce_method == "max":
                return tf.reduce_max(scores, axis=1)
            raise ValueError("Unsupported contrastive_intent_reduce: {}".format(reduce_method))

        def intent_score(intent_set, item_emb):
            return reduce_intent_scores(
                tf.reduce_sum(intent_set * item_emb[:, None, :], -1)
            )
        ...
        if self.hparams.contrastive_loss == 'bpr':
            long_mean_recent_loss = tf.reduce_sum(
                contrastive_mask * tf.math.softplus(
                    intent_score(self.att_fea_long, -self.hist_mean + self.hist_recent)
                )
            ) / tf.reduce_sum(contrastive_mask)
            short_recent_mean_loss = tf.reduce_sum(
                contrastive_mask * tf.math.softplus(
                    intent_score(self.att_fea_short, -self.hist_recent + self.hist_mean)
                )
            ) / tf.reduce_sum(contrastive_mask)
            mean_long_short_loss = tf.reduce_sum(
                contrastive_mask * tf.math.softplus(
                    -intent_score(self.att_fea_long, self.hist_mean)
                    + intent_score(self.att_fea_short, self.hist_mean)
                )
            ) / tf.reduce_sum(contrastive_mask)
            recent_short_long_loss = tf.reduce_sum(
                contrastive_mask * tf.math.softplus(
                    -intent_score(self.att_fea_short, self.hist_recent)
                    + intent_score(self.att_fea_long, self.hist_recent)
                )
            ) / tf.reduce_sum(contrastive_mask)  
        elif self.hparams.contrastive_loss == 'triplet':
            margin = self.hparams.triplet_margin
            distance_long_mean = reduce_intent_scores(
                tf.reduce_sum(tf.square(self.att_fea_long - self.hist_mean[:, None, :]), -1)
            )
            distance_long_recent = reduce_intent_scores(
                tf.reduce_sum(tf.square(self.att_fea_long - self.hist_recent[:, None, :]), -1)
            )
            distance_short_mean = reduce_intent_scores(
                tf.reduce_sum(tf.square(self.att_fea_short - self.hist_mean[:, None, :]), -1)
            )
            distance_short_recent = reduce_intent_scores(
                tf.reduce_sum(tf.square(self.att_fea_short - self.hist_recent[:, None, :]), -1)
            )
            long_mean_recent_loss = tf.reduce_sum(
                contrastive_mask * tf.maximum(0.0, distance_long_mean - distance_long_recent + margin)
            ) / tf.reduce_sum(contrastive_mask)
            short_recent_mean_loss = tf.reduce_sum(
                contrastive_mask * tf.maximum(0.0, distance_short_recent - distance_short_mean + margin)
            ) / tf.reduce_sum(contrastive_mask)
            mean_long_short_loss = tf.reduce_sum(
                contrastive_mask * tf.maximum(0.0, distance_long_mean - distance_short_mean + margin)
            ) / tf.reduce_sum(contrastive_mask)
            recent_short_long_loss = tf.reduce_sum(
                contrastive_mask * tf.maximum(0.0, distance_short_recent - distance_long_recent + margin)
            ) / tf.reduce_sum(contrastive_mask)
    ...
    def _build_seq_graph(self):
        ...
        with tf.variable_scope("clsr"):
            ...
            with tf.variable_scope("long_term"):
                ...
                self.att_fea_long = att_outputs_long  # [B, K, d]
                self.att_fea_long_pool = tf.reduce_sum(att_outputs_long, 1)
                tf.summary.histogram("att_fea_long", self.att_fea_long_pool)
            ...
            with tf.variable_scope("short_term"):
                ...
                self.att_fea_short = att_outputs_short  # [B, M, d]
                self.att_fea_short_pool = tf.reduce_sum(att_outputs_short, 1)
                tf.summary.histogram("att_fea2", self.att_fea_short_pool)
            # ensemble
            with tf.name_scope("alpha"):
                ...
                if not hparams.manual_alpha:
                    if hparams.predict_long_short:
                        ...
                        concat_all = tf.concat(
                            [
                                ...
                                self.att_fea_long_pool,
                                self.att_fea_short_pool,
                    ...
                    user_embed = self.att_fea_long_pool * self.alpha_output + self.att_fea_short_pool * (1.0 - self.alpha_output)
                    ...
                    user_embed = self.att_fea_long_pool * hparams.manual_alpha_value + self.att_fea_short_pool * (1.0 - hparams.manual_alpha_value)
